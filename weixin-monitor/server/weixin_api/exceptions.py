...
# As with any software that integrates with the Weixin platform, your use
# of this software is subject to the Weixin Developer Principles and
...
class WeixinError(Exception):
    """
    All errors specific to Weixin api requests and Weixin ads design will be
    subclassed from WeixinError which is subclassed from Exception.
    """
    pass
...
class WeixinRequestError(WeixinError):
    ...
    WeixinResponse object returned through a callback function (relevant
    ...
    def __init__(
        ...
        super(WeixinRequestError, self).__init__(
            "\n\n" +
            "  Message: %s\n" % self._message +
            "  Method:  %s\n" % request.get('method') +
            "  Path:    %s\n" % request.get('path', '/') +
